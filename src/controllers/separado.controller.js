...
export const getById = async (req, res) => {
    try {
        ...
        if (!categoria) {
            return res.status(404).json({ message: 'Separado no encontrada' });
        }
        res.json(categoria);
    } catch (err) {
        res.status(500).json({ error: err.message });
...
export const create = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: 'Separado creada', id: insertId });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
};
...
export const update = async (req, res) => {
    try {
        ...
        res.json({ message: 'Separado actualizada' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
};
...
export const delete1 = async (req, res) => {
    try {
        ...
        res.json({ message: 'Separado eliminada' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
};
